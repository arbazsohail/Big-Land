...
class RouterHelper {
  ...
  static String getNewPassRoute(
    ...
    String countryCode,
      ...
      _navigateRoute(
          '$createNewPassScreen?email_or_phone=${Uri.encodeComponent(emailOrPhone)}&token=$token&countryCode=$countryCode');
  static String getVerifyRoute(String page, String email,
      {String? session, RouteAction? action, String? countryCode}) {
    String data = Uri.encodeComponent(jsonEncode(email));
    String authSession = base64Url.encode(utf8.encode(session ?? ''));
    if (countryCode == null) {
      var a = '$verify?page=$page&email=$data&data=$authSession';
      return _navigateRoute(a, route: action);
    } else {
      var a =
          // ignore: unnecessary_null_comparison
          '$verify?page=$page&email=$data&data=$authSession${countryCode != null ? '&countryCode=$countryCode' : ''}';
      return _navigateRoute(a, route: action);
    }
  }
